<?php

use App\Http\Controllers\CustomerController;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('auth:api')->get('/user', function (Request $request) {
    return $request->user();
});

Route::post('signIn', [CustomerController::class, 'signIn']);
Route::post('signUp', [CustomerController::class, 'signUp']);

// Route::group(['middleware' => ['auth:api', 'scopes:super_admin']], function () {
// Route::post('dashboard',  [CustomerController::class, 'dashboard']);
Route::post('dashboard',  [CustomerController::class, 'dashboard'])->middleware(['auth:api', 'scopes:super_admin']);
// });
// });
